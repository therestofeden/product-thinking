...
*A walkthrough of the ZMS PRD format I use after discovery is done: seven sections, a mandatory Jobs to be Done gate, and a distinction between release criteria and success metrics that most PRDs collapse into one.*
...
The second is system design language: "This release adds an audit log table to the Ghost Ads configuration service, with a change-tracking layer that persists previous and current state alongside a timestamp and change source." This sentence is accurate. It describes what engineering is building. It is not a product definition — it is a technical specification that has been moved to the wrong document.
...
|---|---|---|---|
| ASM-001 | [KNOWN] | Ghost Ads v1 tests use a fixed configuration after launch — no mid-test changes are permitted by the current UI | No backward-compatibility concern for v1 |
...
- Backward compatibility for all existing Ghost Ads v1 tests
